...
- [ ] **설정 화면 (Settings Screen)**
  - [ ] UI 레이아웃 구현 (기본 설정, 데이터 관리, 앱 정보 섹션)
  - [ ] 일일 예산 설정 다이얼로그 및 기능 연동
  - [ ] 다크 모드 토글 스위치 및 테마 전환 기능 연동
  - [ ] 알림 설정 토글 스위치 기능 연동
  - [ ] 카테고리 관리 페이지로 이동하는 네비게이션 구현
  - [ ] 데이터 초기화 기능 및 확인 다이얼로그 구현
- [ ] **카테고리 관리 화면 (Category Management Screen)**
  - [ ] '필수'/'변동' 탭 UI 구현
  - [ ] 기본 및 사용자 정의 카테고리 목록 표시
  - [ ] 카테고리 추가 다이얼로그 구현
  - [ ] 카테고리 수정/삭제 기능 구현 (스와이프 액션 등)
- [ ] **메인 화면 (Home Screen)**
  - [ ] UI 레이아웃 구현 (날짜, 원형 차트, 요약 정보)
  - [ ] 일일 지출 현황 데이터 연동 및 원형 차트 시각화
  - [ ] '오늘의 지출 보기' Bottom Sheet 구현 및 데이터 연동
  - [ ] '지출 등록하기' Bottom Sheet 구현
- [ ] **지출 등록/수정 기능**
  - [ ] 지출 등록/수정 폼 위젯 구현 (날짜 선택, 금액/이름 입력, 카테고리 선택)
  - [ ] 폼 상태 관리 및 유효성 검사 로직 구현
  - [ ] `ExpenseRepository`와 연동하여 데이터 저장/수정
- [ ] **캘린더 화면 (Calendar Screen)**
  - [ ] 캘린더 UI 라이브러리 선정 또는 직접 구현
  - [ ] 월별 이동 기능 구현
  - [ ] 각 날짜 셀에 총지출액 및 목표 달성 여부 시각화
  - [ ] 월간 통계 데이터 표시
  - [ ] 날짜 클릭 시 해당일 지출 내역 Bottom Sheet 표시
- [ ] **지출 내역 화면 (Expenses List Screen)**
  - [ ] 날짜별로 그룹화된 지출 목록 UI 구현
  - [ ] 무한 스크롤 또는 페이지네이션을 통한 과거 데이터 로딩 구현

---

## ☁️ Phase 2: 백엔드 연동 (Supabase)

- [ ] **Supabase 테이블 및 정책 설정**
  - [ ] `DEVELOPMENT_SPEC.md`에 따라 `users`, `categories`, `expenses` 테이블 생성
  - [ ] 각 테이블에 RLS(Row Level Security) 정�� 설정
  - [ ] 기본 카테고리 데이터 `seed.sql` 스크립트로 추가
- [ ] **인증 로직 구현**
  - [ ] `AuthRepository`에 Supabase 익명 로그인(`signUp`) 로직 추가
  - [ ] 소셜 로그인(Google, Apple) 및 계정 연동(`updateUser`) 로직 구현
- [ ] **리포지토리 수정**
  - [ ] 각 리포지토리의 데이터 소스를 `sqflite`에서 Supabase 클라이언트로 교체
  - [ ] 오프라인 상태를 대비한 캐싱 및 동기화 로직 추가 (선택 사항)
- [ ] **데이터 마이그레이션**
  - [ ] 기존 로컬 DB 사용자가 처음 온라인 전환 시, 데이터를 Supabase로 이전하는 로직 구현
