...
| ID   | 문제 / 개선 포인트                                                                                                                                                                                                                                          | 근거                                                              | 작업 날짜 | 해결 상태 |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | --------- | --------- |
| rf_1 | 네비게이션 초기화: `ref.listen(homeViewModelProvider, …)`를 `SplashScreen` `build()` 안에서 등록해 빌드마다 리스너가 중첩되고 다중 `context.go`가 발생할 수 있음. `ConsumerStatefulWidget`으로 승격해 `initState`에서 단 한 번만 listen 하도록 리팩터 필요. | `lib/features/auth/view/splash_screen.dart:21-38`                 | -         | x         |
| rf_2 | 리뷰 요청: 부정 피드백 전송 시 `Supabase.from('app_feedback')` insert payload가 `{'userid: <uuid>'}` 꼴로 들어가 `column "<uuid>" does not exist` 오류 위험. 키를 `'user_id'`로 고정하고 예외 로깅 추가 필요.                                               | `lib/core/services/review_prompt_service.dart:134-145`            | -         | x         |
