...
> **역사적 기준선 문서**: 이 문서는 리팩터링 시작 전(`main`의
> `4ebbddd`) 분석을 보존한다. PR 7.4까지의 구현 상태와 현재 공개 경계는
> [루트 README](../../../README.md), [기능 소유권](../../architecture/feature-owners.md),
> [ADR](../../adr/README.md)를 기준으로 한다.

> 분석 기준: 2026-07-21, `main` 브랜치 `4ebbddd`  
> 범위: `lib/`, `test/`, 모바일 빌드·배포 설정, 자산, 기존 아키텍처 문서  
> 성격: 리팩터링 시작 시점 코드에 대한 정적 분석과 개선 설계 문서다. 이후 구현은 별도
> 커밋과 현재 아키텍처 문서에 기록한다.
