...
const positiveBigintPattern = /^[1-9][0-9]{0,18}$/;
const maxBigint = 9223372036854775807n;
const controlCharacters = /[\u0000-\u001f\u007f]/g;
const likelyPii =
  /(?:\b[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}\b|\b(?:\d[ -]?){12,19}\b|\b\d{2,4}[ -]?\d{3,4}[ -]?\d{4}\b)/;

export function isSupportedDeliveryId(value: string): boolean {
  if (uuidPattern.test(value)) return true;
  if (!positiveBigintPattern.test(value)) return false;
...
export function retryAt(
  ...
  return new Date(
    now.getTime() + Math.max(exponentialMs, requestedMs) + jitterMs,
  ).toISOString();
}
...
export function slackPayload(
  ...
  const title = isInquiry
    ? `New MoneyFit inquiry · ${inquiryLabel(row.inquiry_type)}`
    : "New MoneyFit feedback";
  ...
  const metadata = [
    ...
    `version ${safePlainText(row.app_version, 40) || "unknown"}${
      row.build_number ? ` (${safePlainText(row.build_number, 40)})` : ""
    }`,
  ...
  if (isInquiry && row.email) {
    blocks.push({
      type: "section",
      text: plainText(`Reply email: ${safePlainText(row.email, 254)}`),
    });
  }
...
function retryIsDue(row: DeliveryRow, now: Date): boolean {
  if (row.slack_status !== "failed" || !row.slack_next_retry_at) return true;
  const scheduled = Date.parse(row.slack_next_retry_at);
...
export async function deliver(
  ...
  if (!deliveryTables.has(table) || !isSupportedDeliveryId(id)) {
  ...
  if (row.slack_status === "processing") {
    await store.recoverStale(
      table,
      id,
      new Date(now.getTime() - staleProcessingMs).toISOString(),
    );
    row = await store.read(table, id);
    if (!row) return { kind: "not_found", status: 404 };
  }
  if (
    !isClaimable(row) || !retryIsDue(row, now) ||
    (row.slack_attempts ?? 0) >= maxDeliveryAttempts
  ) {
    if (
      (row.slack_attempts ?? 0) >= maxDeliveryAttempts &&
      row.slack_status !== "sent"
    ) {
      await store.update(table, id, {
        slack_status: "failed",
        slack_next_retry_at: null,
        slack_last_error_code: "retry_exhausted",
      });
    }
  ...
  if (
    ...
    await store.update(table, id, {
      ...
      slack_last_error_code: "rate_limited",
      slack_next_retry_at: null,
    });
    return { kind: "noop", status: 200 };
  }
  ...
  if (!webhook) {
    await store.update(table, id, {
      slack_status: "suppressed",
      slack_last_error_code: "webhook_not_configured",
      slack_next_retry_at: null,
    });
    return { kind: "disabled", status: 200 };
  }
  try {
    // Use the event ID, not a JSON-decoded bigint from the canonical row, for
    // Slack metadata. JavaScript numbers cannot exactly represent every bigint.
    const response = await dependencies.postSlack(
      webhook,
      slackPayload(table, { ...row, id }),
    );
    if (response.status === 200 && response.body.trim() === "ok") {
      await store.update(table, id, {
        slack_status: "sent",
        slack_notified_at: now.toISOString(),
        slack_next_retry_at: null,
        slack_last_error_code: null,
      });
    ...
    if (response.status === 429 || response.status >= 500) {
      await store.update(table, id, {
        slack_status: "failed",
        slack_last_error_code: response.status === 429
          ? "http_429"
      ...
      await store.update(table, id, {
        slack_status: "suppressed",
        slack_last_error_code: `http_${response.status}`,
        slack_next_retry_at: null,
      });
    }
  } catch (_) {
    await store.update(table, id, {
      slack_status: "failed",
      slack_last_error_code: "timeout_or_network",
      slack_next_retry_at: retryAt(attempt, now),
    });
  }
